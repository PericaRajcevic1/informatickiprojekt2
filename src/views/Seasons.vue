<template>
  <v-container>
    ...
    <h4 style="font-size: 2rem" class="text-center">Sezone</h4>
    ...
    <v-container v-if="model">
      <v-chip-group>
        <v-chip v-if="drivers[0] && drivers[0].points > 0">
          ...
          Svjetski šampion : {{ drivers[0].Driver.givenName }} {{ drivers[0].Driver.familyName }}

        </v-chip>
        <v-chip v-if="constructors[0] && constructors[0].points > 0">
          <v-icon>mdi-trophy-award</v-icon>
          Konstruktorski šampion : {{ constructors[0].Constructor.name }}
        </v-chip>
      </v-chip-group>
                    ...
                    <v-list-item three-line>
                      <v-list-item-content>
                        <div class="text-overline mb-4">
                          ...
                          {{
                            new Date(race.date).toLocaleDateString("en-DE", {
                              year: "numeric",
                              month: "long",
                              day: "numeric"
                            })
